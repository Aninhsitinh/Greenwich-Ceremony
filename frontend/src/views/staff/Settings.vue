<template>
  ...
  >
    <div class="w-full max-w-4xl mx-auto px-4 py-6 space-y-6">
      ...
      <div class="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-lg border border-gray-200 dark:border-gray-700">
        <div class="flex flex-col md:flex-row items-center gap-8">
          ...
          <div class="relative group">
            <div class="w-32 h-32 rounded-full p-1 bg-gradient-to-tr from-orange-500 to-rose-600">
              <div 
      ...
      <div v-if="isEditing" class="bg-white dark:bg-gray-800 rounded-2xl p-8 shadow-lg border border-gray-200 dark:border-gray-700 animate-fade-in">
        ...
        <form @submit.prevent="saveProfile" class="space-y-6">
          <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <label class="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-2">Job Title</label>
              <input
      ...
      <!-- System Settings Section Removed -->
...
const avatarStyle = computed(() => {
  ...
  return `background-image: url('https://ui-avatars.com/api/?name=${profile.value?.fullName || 'Staff'}&background=111827&color=fff&size=200')`;
});
...
const startEdit = () => {
  editForm.value = {
    ...
    jobTitle: profile.value?.jobTitle || 'Mentor'
  };
...
const cancelEdit = () => {
  ...
  editForm.value = { fullName: '', email: '', phone: '', jobTitle: '' };
};
...
const saveProfile = async () => {
  saving.value = true;
  try {
    const response = await api.patch('/staff/profile', editForm.value);
    if (response.data.success) {
      profile.value = response.data.data.user;
